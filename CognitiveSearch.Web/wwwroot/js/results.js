...
function UpdateResults(data, q) {
    ...
    $("#doc-count").html(` 搜索结果个数: ${data.count}`);
...
function UpdatePictures(pictures) {
    ...
    if (currentDocCount > pictures.length) {
        currentDocCount = pictures.length;
    }

    $("#doc-count").html(` 搜索结果个数: ${pictures.length}`);

    for (var i = 0; i < pictures.length; i++) {
        var result = pictures[i].document;
        result.idx = i;
        if (typeof result.tagDisplay === 'undefined') {
            result.tagDisplay = 'none';
        }
...
function UpdateFiles(files) {
    ...
    if (currentDocCount > files.length) {
        currentDocCount = files.length;
    }

    $("#doc-count").html(` 搜索结果个数: ${files.length}`);

    for (var i = 0; i < 10; i++) {

        var result = files[i+(currentPage-1)*10].document;
        result.idx = i;
        if (typeof result.tagDisplay === 'undefined') {
            result.tagDisplay = 'none';
        }
