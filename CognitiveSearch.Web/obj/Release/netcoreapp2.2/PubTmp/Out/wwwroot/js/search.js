...
function addPicture(totalData) {
    for (var i = 1; i < (parseInt(totalData) / 10 + 1); i++) {
        $.ajax({
            url: `${apiUrl}/documents/`,
            type: 'POST',
            contentType: 'application/json; charset=utf-8',
            ...
            success: function (data) {
                if (document.getElementById("results-entity-map").style.display === "none") {
                    ...
                    for (var j = 0; j < data.results.length; j++) {
                        var path = data.results[j].document.metadata_storage_path + token;
                        var pathLower = path.toLowerCase();
                        if (pathLower.includes(".jpg") || pathLower.includes(".png")) {
                            pictures.push(data.results[j]);
                        }
                    }
                } else {
                    LoadEntityMap();
            ...
            complete: function () {
                ...
                UpdatePagination(pictures.length, 1);
            }
        });
    }
}

function addFile(totalData) {
    for (var i = 1; i < (parseInt(totalData) / 10 +1); i++) {
        $.ajax({
            url: `${apiUrl}/documents/`,
            type: 'POST',
            ...
            success: function (data) {
                if (document.getElementById("results-entity-map").style.display === "none") {
                    ...
                    for (var j = 0; j < data.results.length; j++) {
                        var path = data.results[j].document.metadata_storage_path + token;
                        var pathLower = path.toLowerCase();
                        if (pathLower.includes(".jpg") || pathLower.includes(".png") || pathLower.includes(".mp3") || pathLower.includes(".mp4")) {
                        } else {
                            files.push(data.results[j]);
                        }
                    }
                } else {
                    LoadEntityMap();
            ...
            complete: function () {
                UpdateFiles(files);
                UpdatePagination(files.length, 2);
            }
        });
    }
}
...
function UpdatePagination(docCount, s) {
    ...
    var htmlString = "页数: ";
    if (currentPage > 1) {
        htmlString += `<li><a href="javascript:void(0)" onclick="GoToPage('${backPage}', '${s}')" class="ms-Icon ms-Icon--ChevronLeftMed"></a></li>`;
    }

    htmlString += '<li class="active"><a href="#">' + currentPage + '</a></li>';

    if (currentPage <= totalPages) {
        htmlString += `<li><a href="javascript:void(0)" onclick="GoToPage('${forwardPage}', '${s}')" class="ms-Icon ms-Icon--ChevronRightMed"></a></li>`;
    }
    ...
    htmlString += '&nbsp;&nbsp;[第 ' + firstDoc + ' 到第 ' + lastDoc+'个文件]';
    $("#pagination").html(htmlString);
    $("#paginationFooter").html(htmlString);
}

function GoToPage(page, s) {
    //0: Search all; 1: Search Pictures; 2: Search Files
    currentPage = page;
    if (s == 0) {
        Search();
    } else if (s == 1) {
